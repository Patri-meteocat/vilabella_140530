...
out_path = '/Users/patriciaaltube/Desktop/Vilabella_Joan/plots_Z/'
...
for f in glob.glob(data_path + '*.pkl'):
	...
	for nsw, sweep_slice in enumerate(radar.iter_slice()):
		...
		out_pdf = out_path + file_name + '_' + str(nsw) + '_Z.png'
		...
		# Display reflectivity, (vmin, vmax) are colorbar limits
 	   	display.plot('reflectivity', nsw, ax=ax, mask_outside=False, vmin=-10, vmax=60., colorbar_flag=True, title_flag=False, colorbar_label='[dBZ]')
		# Display range rings
		...
		cbar = display.cbs
		cbar[0].set_label('Reflectivity [dBZ]', fontsize=14)
